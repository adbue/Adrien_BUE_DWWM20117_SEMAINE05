...
class Produits extends CI_Controller 
{
    ...
    public function ajouter()
    {
        ...
        if ($this->input->post()) 
        {
            ...
            $this->form_validation->set_rules("pro_ref","Référence", "required|min_length[6]", array("required" =>"Veuillez renseigner une %s.", "min_length[6]" =>"Le %s doit avoir longueur minimum de 6 caractères."));
            $this->form_validation->set_rules("pro_libelle","Libellé", "required|alpha_numeric", array("required" =>"Veuillez renseigner une %s.", "min_length[6]" =>"Le champ %s n'accepte pas de chiffres."));
            $this->form_validation->set_rules("pro_cat_id","Catégorie", "required", array("required" =>"Veuillez sélectionner une %s."));
            $this->form_validation->set_rules("pro_description","Description", "required", array("required" =>"Veuillez renseigner une %s.", "min_length[6]" =>"Le %s doit avoir longueur minimum de 6 caractères."));
            $this->form_validation->set_rules("pro_prix","Prix", "required|decimal", array("required" =>"Veuillez renseigner une %s.", "min_length[6]" =>"Le champ %s doit être complété avec un chiffre décimal."));
            $this->form_validation->set_rules("pro_stock","Stock", "required|integer", array("required" =>"Veuillez renseigner une %s.", "min_length[6]" =>"Le champ %s doit être complété avec un chiffre entier."));
            $this->form_validation->set_rules("pro_couleur","Couleur", "required|alpha", array("required" =>"Veuillez renseigner une %s.", "min_length[6]" =>"Le champ %s ne doit comporter que des lettres."));
        ...
        { // Le formulaire n'est pas posté on retourne sur la page ajouter
            $this->load->view('header');
    ...
    {
        $this->load->model('ProduitsModel');
        $aView['produit'] = $this->ProduitsModel->listeDetail($id);

        $this->load->view('header');
        $this->load->view('detail', $produit);
        $this->load->view('footer');
    }
